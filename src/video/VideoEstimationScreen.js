				...
				if (keypoint.score >= 0.8) {
					return (
						<Circle
							key={index}
							r={3}
							cx={keypoint.position.x * (400 / image.width)}
							cy={keypoint.position.y * (400 / image.height)}
							fill="white"
						/>
					);
				}
			...
			return skeletonPairs.map((pair, index) => {
				return (
					<Line
						...
						x1={
							pose.keypoints.find(
								keypoint => keypoint.part == pair.pair[0]
							).position.x *
							(400 / image.width)
						}
						x2={
							pose.keypoints.find(
								keypoint => keypoint.part == pair.pair[1]
							).position.x *
							(400 / image.width)
						}
						y1={
